...
export function PipelineResetDialog() {
  ...
  const executeReset = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("reset-pipeline", {
        body: { password, dry_run: false },
      });
      if (error) throw error;
      if (data?.errors) {
        toast.error(`Reset completed with errors: ${data.errors.join(", ")}`);
      } else {
        toast.success("Pipeline reset complete. Ready for fresh test run.");
      }
  ...
  return (
    <>
      <Button
        ...
        size="sm"
        className="text-xs"
        onClick={() => { setPasswordOpen(true); setPassword(""); setPwError(false); }}
      >
