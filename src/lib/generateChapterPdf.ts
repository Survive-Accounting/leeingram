...
function sectionHeader(doc: jsPDF, y: number, title: string, chapterName: string, pageRef: { num: number }): number {
  y = ensureSpace(doc, y, 15, chapterName, pageRef);
  doc.setFontSize(14);
...
export function generateChapterPdf(data: ChapterPdfData) {
  ...
  doc.setTextColor(NAVY);
  doc.setFont("helvetica", "bold");
  doc.text(`Chapter ${data.chapterNumber}: ${data.chapterName}`, MARGIN_LEFT, y);
  y += 8;
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  ...
  if (purposeBullets.length > 0 || consequenceBullets.length > 0) {
    ...
    if (purposeBullets.length > 0) {
      ...
      for (const bullet of purposeBullets) {
        const lines = wrapText(doc, `• ${bullet}`, CONTENT_WIDTH - 5);
  ...
  if (data.keyTerms.length > 0) {
    ...
    for (const [cat, terms] of categories) {
      ...
      for (const t of terms) {
        const defLines = wrapText(doc, t.definition, CONTENT_WIDTH - 30);
        const needed = Math.max(defLines.length * LINE_HEIGHT, LINE_HEIGHT) + 3;
        y = ensureSpace(doc, y, needed, data.chapterName, pageRef);
        doc.setFont("helvetica", "bold");
        doc.text(t.term, MARGIN_LEFT + 2, y);
        doc.setFont("helvetica", "normal");
        for (let i = 0; i < defLines.length; i++) {
          doc.text(defLines[i], MARGIN_LEFT + 2, y + (i > 0 ? i * LINE_HEIGHT : LINE_HEIGHT));
        }
        y += LINE_HEIGHT + defLines.length * LINE_HEIGHT + 1;
      }
      y += 2;
    }
  }
  ...
  if (data.accounts.length > 0) {
    ...
    for (const groupName of ACCOUNT_TYPE_ORDER) {
      ...
      for (const acc of groupAccounts) {
        const descLines = wrapText(doc, acc.account_description, CONTENT_WIDTH - 10);
        ...
        doc.setFont("helvetica", "normal");
        doc.setTextColor(100, 100, 100);
        doc.text(`Normal balance: ${acc.normal_balance}`, MARGIN_LEFT + 3 + doc.getTextWidth(acc.account_name) + 4, y);
        doc.setTextColor(40, 40, 40);
        y += LINE_HEIGHT;
  ...
  if (data.formulas.length > 0) {
    ...
    for (const f of data.formulas) {
      const expLines = wrapText(doc, f.formula_expression, CONTENT_WIDTH - 10);
      const explLines = f.formula_explanation ? wrapText(doc, f.formula_explanation, CONTENT_WIDTH - 10) : [];
  ...
  if (data.jeEntries.length > 0) {
    ...
    const renderJE = (je: JEEntry) => {
      ...
      const needed = LINE_HEIGHT + rowCount * LINE_HEIGHT + 6;
      y = ensureSpace(doc, y, needed, data.chapterName, pageRef);

      doc.setFont("helvetica", "bold");
      doc.setTextColor(40, 40, 40);
      doc.setFontSize(10);
      const labelLines = wrapText(doc, je.transaction_label, CONTENT_WIDTH - 5);
      for (const l of labelLines) {
        doc.text(l, MARGIN_LEFT + 3, y);
        y += LINE_HEIGHT;
      }

      doc.setFont("helvetica", "normal");
      doc.setFontSize(9);
      const colAccount = MARGIN_LEFT + 5;
      const colDebit = MARGIN_LEFT + CONTENT_WIDTH * 0.65;
      const colCredit = MARGIN_LEFT + CONTENT_WIDTH * 0.82;

      // Header row
      doc.setTextColor(100, 100, 100);
      doc.text("Account", colAccount, y);
      doc.text("Debit", colDebit, y, { align: "right" });
      doc.text("Credit", colCredit, y, { align: "right" });
      y += 1;
      doc.setDrawColor(180, 180, 180);
      doc.setLineWidth(0.2);
      doc.line(colAccount, y, MARGIN_LEFT + CONTENT_WIDTH - 5, y);
      y += LINE_HEIGHT - 1;
      ...
      for (const line of lines) {
        ...
        doc.text(isCredit ? `    ${line.account}` : line.account, colAccount, y);
        doc.text(isCredit ? "" : "???", colDebit, y, { align: "right" });
        doc.text(isCredit ? "???" : "", colCredit, y, { align: "right" });
        y += LINE_HEIGHT;
      }
      y += 3;
    };
  ...
  if (data.mistakes.length > 0) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      const m = sorted[i];
      const mistakeLines = wrapText(doc, m.mistake, CONTENT_WIDTH - 10);
      const explLines = m.explanation ? wrapText(doc, m.explanation, CONTENT_WIDTH - 10) : [];
