...
5. ENTITY NAMING: The primary entity is always "Survive Company A ([role])". A second entity is "Survive Company B ([role])". ONLY use these 7 roles: seller, buyer, manufacturer, borrower, lender, lessee, lessor. Never use "Survive Company" without A/B suffix and role hint. Never use "Counterparty" or X/Y naming. Include the parenthetical role hint ONLY on the very first mention of each entity; omit it on all subsequent mentions.
6. Return only JSON.`;
...
6. ENTITY NAMING: The primary entity is always "Survive Company A ([role])". A second entity is "Survive Company B ([role])". ONLY use these 7 roles: seller, buyer, manufacturer, borrower, lender, lessee, lessor. Never use "Survive Company" without A/B suffix and role hint. Never use "Counterparty" or X/Y naming. Always state which entity's books the entry is recorded on using "on the books of Survive Company A/B". Include the parenthetical role hint ONLY on the very first mention of each entity; omit it on all subsequent mentions.
7. Return only JSON.`;
