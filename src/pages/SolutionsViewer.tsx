...
import { isCanonicalJE, type CanonicalJEPayload, computeBalance, formatAmount } from "@/lib/journalEntryParser";
...
function PipeTable({ rows }: { rows: string[][] }) {
  ...
  return (
    <div className="my-4 relative">
      <button
        onClick={copyTSV}
        className="absolute top-2 right-2 text-[10px] px-2 py-1 rounded bg-white border border-gray-200 text-gray-500 hover:text-gray-800 hover:bg-gray-50 transition-colors z-10"
      >
        Copy as TSV
      </button>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr style={{ background: "#1A2E55" }}>
              {header.map((h, i) => (
                <th key={i} className="px-3 py-2 text-center text-white font-bold text-[13px]">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {body.map((row, ri) => (
              <tr key={ri} style={{ background: ri % 2 === 0 ? "#FFFFFF" : "#F8F9FA" }}>
                {row.map((cell, ci) => (
                  <td key={ci} className={`px-3 py-1.5 text-center text-[13px] ${isNumericCell(cell) ? "font-mono" : ""}`}>{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h2
      className="text-[11px] font-bold tracking-[0.15em] uppercase border-b border-gray-200 pb-1 mb-3 mt-8"
      style={{ color: "#131E35" }}
    >
      {children}
    </h2>
    ...
    <div className="space-y-4">
      {entries.map((entry: any, ei: number) => {
        ...
        return (
          <div key={ei}>
            {date && (
              <p className="font-bold text-sm mb-1" style={{ color: "#131E35" }}>{date}</p>
            )}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map((row: any, ri: number) => {
                    ...
                    return (
                      <tr key={ri} style={{ background: ri % 2 === 0 ? "#FFFFFF" : "#F8F9FA" }}>
                        <td className={`px-3 py-1.5 text-[13px] ${isCredit ? "pl-10" : ""}`}>
                          {row.account_name || row.account || ""}
                        </td>
                        <td className="text-right px-3 py-1.5 text-[13px] font-mono">
                          {!isCredit && row.debit != null && row.debit !== 0 ? Number(row.debit).toLocaleString("en-US") : ""}
                        </td>
                        <td className="text-right px-3 py-1.5 text-[13px] font-mono">
                          {isCredit && row.credit != null ? Number(row.credit).toLocaleString("en-US") : ""}
                        </td>
                      </tr>
                    );
                  })}
...
// ── Answer Summary formatter ─────────────────────────────────────────

function AnswerSummarySection({ text }: { text: string }) {
  // Split by sub-section labels like (a), (b), (c)
    ...
    <div className="rounded-md p-4 pl-5 border-l-[3px]" style={{ background: "#F0FFF4", borderColor: "#1B8A3E" }}>
      {subSections.map((section, si) => {
        ...
        return (
          <div key={si}>
            {label && (
              <p className="font-bold text-[15px] mt-4 first:mt-0" style={{ color: "#131E35" }}>{label}</p>
            )}
            {lines.map((line, li) => {
              const yearMatch = line.trim().match(/^(\d{4})\s*:/);
              if (yearMatch) {
                return (
                  <p key={li} className="font-bold text-[14px] mt-2" style={{ color: "#1A2E55" }}>
                    {line.trim()}
                  </p>
                );
              }
              return (
                <p key={li} className="text-[13px] ml-4 mb-1" style={{ color: "#1A1A1A" }}>
                  {line.trim()}
                </p>
              );
            })}
      ...
      <div className="absolute inset-0 backdrop-blur-md z-10 rounded-lg flex items-center justify-center" style={{ background: "rgba(19, 30, 53, 0.85)" }}>
        <div className="text-center px-8 py-10 max-w-md">
          ...
          <a
            ...
            className="inline-block mt-4 px-6 py-3 rounded-lg text-white font-bold text-sm transition-all hover:scale-105"
            style={{ background: "#1B8A3E" }}
          >
            Get Study Pass →
          </a>
        </div>
...
export default function SolutionsViewer() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (asset.course_id) {
        const { data: c } = await supabase.from("courses").select("*").eq("id", asset.course_id).single();
        course = c;
      }
      if (asset.chapter_id) {
        const { data: ch } = await supabase.from("chapters").select("*").eq("id", asset.chapter_id).single();
        chapter = ch;
      }
  ...
  useEffect(() => {
    ...
    const key = `solutions_viewed_${data.asset.id}`;
    if (sessionStorage.getItem(key)) return;
    sessionStorage.setItem(key, "1");
    supabase.rpc("increment_solutions_views", { asset_id: data.asset.id }).then(() => {});
  }, [data?.asset?.id]);
      ...
      <header className="border-b-2" style={{ borderColor: "#131E35" }}>
        <div className="max-w-[780px] mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-[12px]">
            {quizLink && (
              <a href={quizLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
                Practice Quiz <ExternalLink className="h-3 w-3" />
              </a>
            )}
            {whiteboardLink && (
              <a href={whiteboardLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
                Whiteboard <ExternalLink className="h-3 w-3" />
              </a>
            )}
            {videoLink && (
              <a href={videoLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
      ...
      <div style={{ background: "#F8F9FA" }} className="border-b border-gray-200">
        <div className="max-w-[780px] mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <a
              href={`/practice/${asset.asset_name}`}
      ...
      <main className="max-w-[780px] mx-auto px-6 py-8">
              ...
              {sortedInstr.filter(i => i.instruction_text?.trim()).map((inst, idx) => {
                ...
                return (
                  <p key={idx} className="text-[14px] leading-[1.6]">
                    <span className="font-bold" style={{ color: "#131E35" }}>({letter})</span>{" "}
        ...
        {isPreview ? (
          ...
          <>
            ...
            {hasJE && (
              <>
                <SectionHeading>JOURNAL ENTRIES</SectionHeading>
                {hasCanonicalJE ? (
            ...
            {answerSummary.trim() && (
              <>
                <SectionHeading>ANSWER SUMMARY</SectionHeading>
            ...
            {flowchartUrl && (
              <>
                <SectionHeading>HOW TO SOLVE THIS</SectionHeading>
                <div className="text-center">
            ...
            {examTraps.trim() && (
              <>
                <SectionHeading>⚠ EXAM TRAPS</SectionHeading>
