...
import { ExternalLink, Copy, Mail, Globe } from "lucide-react";
...
const COURSE_TABS = [
  { code: "accy201", label: "ACCY 201", dbCode: "FA1", chapterRange: [1, 12] },
  { code: "accy202", label: "ACCY 202", dbCode: "MA1", chapterRange: [1, 10] },
  { code: "accy303", label: "ACCY 303", dbCode: "IA1", chapterRange: [1, 11] },
  { code: "accy304", label: "ACCY 304", dbCode: "IA2", chapterRange: [13, 22] },
] as const;

function CoursePanel({ tab }: { tab: typeof COURSE_TABS[number] }) {
  const qc = useQueryClient();
  const [enrollInput, setEnrollInput] = useState("");
  const [enrollLoaded, setEnrollLoaded] = useState(false);

  const settingKey = `learnworlds_enroll_url_${tab.code}`;

  const { data: currentEnrollUrl } = useQuery({
    queryKey: ["app-setting", settingKey],
    queryFn: async () => {
      // Try course-specific key first, fall back to generic for accy304
      const { data } = await supabase
        .from("app_settings")
        .select("value")
        .eq("key", tab.code === "accy304" ? "learnworlds_enroll_url" : settingKey)
        .maybeSingle();
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const key = tab.code === "accy304" ? "learnworlds_enroll_url" : settingKey;
      const { error } = await supabase
        .from("app_settings")
        .upsert({ key, value: enrollInput, updated_at: new Date().toISOString() }, { onConflict: "key" });
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["app-setting", settingKey] });
  ...
  const { data: courseData } = useQuery({
    queryKey: ["website-admin-course", tab.dbCode],
    queryFn: async () => {
      const { data: courses } = await supabase.from("courses").select("id").eq("code", tab.dbCode).limit(1);
      return courses?.[0] || null;
    },
  });

  const { data: chapters } = useQuery({
    queryKey: ["website-admin-chapters", courseData?.id, tab.chapterRange],
    queryFn: async () => {
      const { data } = await supabase
        .from("chapters")
        .select("id, chapter_number, chapter_name")
        .eq("course_id", courseData!.id)
        .gte("chapter_number", tab.chapterRange[0])
        .lte("chapter_number", tab.chapterRange[1])
  ...
  const { data: assets } = useQuery({
    queryKey: ["website-admin-assets", courseData?.id],
    queryFn: async () => {
      const { data } = await supabase
  ...
  const landingPath = `/ole-miss/${tab.code}`;
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-3">
          <Button variant="outline" size="sm" onClick={() => copyToClipboard(`${APP_URL}${landingPath}`, "Landing page URL")}>
          ...
          <Button
            ...
            onClick={() =>
              copyToClipboard(
                `<iframe src="${APP_URL}${landingPath}" width="100%" height="900" frameborder="0" style="border:none;border-radius:8px"></iframe>`,
                "iFrame code"
              )
            }
          >
            <Copy className="h-3.5 w-3.5 mr-1.5" /> Copy iFrame
          </Button>
          <Button variant="outline" size="sm" onClick={() => window.open(landingPath, "_blank")}>
          ...
          <Table>
            ...
            <TableBody>
              {chapters?.map((ch) => {
                ...
                return (
                  <TableRow key={ch.id}>
                    <TableCell className="text-sm font-medium">Ch {ch.chapter_number} — {ch.chapter_name}</TableCell>
                    <TableCell className="text-center text-sm">{stats.count}</TableCell>
                    <TableCell className="text-center">
                      {stats.hasSolutions ? <span className="text-emerald-500">✓</span> : <span className="text-red-400">✗</span>}
                    </TableCell>
                    <TableCell className="text-center text-sm text-muted-foreground">{stats.avgViews}</TableCell>
                    <TableCell className="text-right">
                      <Button variant="ghost" size="sm" className="text-xs h-7" onClick={() => window.open(`${landingPath}?chapter=${ch.id}`, "_blank")}>
                        Preview →
                      </Button>
                    </TableCell>
                  </TableRow>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3 items-end">
          <div className="flex-1">
            ...
            <Input value={enrollInput} onChange={(e) => setEnrollInput(e.target.value)} placeholder="https://school.surviveaccounting.com/course/..." className="text-sm" />
          </div>
...
export default function WebsiteAdmin() {
  const [activeTab, setActiveTab] = useState("accy304");

  return (
    <SurviveSidebarLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
          <Globe className="h-5 w-5 text-primary" /> Website Admin
        </h1>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {COURSE_TABS.map((t) => (
            <TabsContent key={t.code} value={t.code} className="mt-4">
              <CoursePanel tab={t} />
            </TabsContent>
          ))}
        </Tabs>
