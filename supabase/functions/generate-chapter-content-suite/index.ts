...
For non-contra accounts, set contra_tooltip and fs_placement_tooltip to null. Mark all as suggested: true.`;
  ...
  if (items.length > 0) {
    const rows = items.map((item: any, i: number) => ({
      ...
      debit_tooltip: item.debit_tooltip || null,
      credit_tooltip: item.credit_tooltip || null,
      balance_tooltip: item.balance_tooltip || null,
      contra_tooltip: item.contra_tooltip || null,
      fs_placement_tooltip: item.fs_placement_tooltip || null,
      example_beginning_balance: item.example_beginning_balance ?? null,
      example_debit_amount: item.example_debit_amount ?? null,
      example_credit_amount: item.example_credit_amount ?? null,
      example_ending_balance: item.example_ending_balance ?? null,
      example_date_label: item.example_date_label || null,
      sort_order: item.sort_order ?? i + 1,
      generated_at: new Date().toISOString(),
    }));
